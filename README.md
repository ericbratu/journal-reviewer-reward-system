...
Miners submit reviews: Each miner sends a historical review from their dataset
Validator groups by paper: Reviews for the same paper are scored together
LLM scores each review: Using 6-criteria rubric (comprehension, technical depth, etc.)
Rankings determine rewards: Top-ranked reviews get exponentially higher rewards
Scores update: Validator updates miner scores using exponential moving average
Monitoring
...
Skip validator, just run miners
Or mock the LLM scorer (returns random scores)
Or use cached scores from previous runs
Next Steps
...
Check logs in ~/.bittensor/miners/ and ~/.bittensor/validators/
Review DATA_PIPELINE_README.md for detailed setup
See architectural plan in project documentation
